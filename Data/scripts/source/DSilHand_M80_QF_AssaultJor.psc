...
Scriptname DSilHand_M80_QF_AssaultJor Extends Quest Hidden
...
; this stage must:
; (1) make the Silver Hand and the Companions friends
; (2) Stop the fight
; (3) start the second scene
Debug.Trace(THIS_FILE + "#STAGE 60")

; temporally disable the player control -- to avoid trouble
Debug.Trace(THIS_FILE + "-- Game.DisablePlayerControls(true, true)")
Game.DisablePlayerControls(true, true)
Game.ForceThirdPerson()

; Make the Factions friendly
Debug.Trace(THIS_FILE + "-- Make Companions and SilverHand friendly")
kmyQuest.endSilverHandCompanionsWar()

; setup armistice scene elements
Debug.Trace(THIS_FILE + "-- setup Scene: kmyQuest.prepareScenePeace()")
kmyQuest.prepareScenePeace()


Debug.Trace(THIS_FILE + "-- run Scene: DSilHand_M80_SceneArmistice2.Start()")
...
Debug.Trace(THIS_FILE + "-- Game.EnablePlayerControls()")
Utility.Wait(3)
...
Debug.Trace(THIS_FILE + "#STAGE 102")
...
; Emulates the fragment collection and dead bodies cleanup
Debug.Trace(THIS_FILE + "#STAGE 90")
...
Debug.Trace(THIS_FILE +  "-- start stage 50: Fight the companions and clean the circle.")
;END CODE
EndFunction
;END FRAGMENT
...
; starting stage 20: Meet the men on the secret passage beneath the Underforge outside of Whiterun and talk with Fjol.
...
Debug.Trace(THIS_FILE + "#STAGE 62")
;END CODE
EndFunction
;END FRAGMENT
...
Debug.Trace(THIS_FILE + "#STAGE 91")
Debug.Trace(THIS_FILE + "-- starting stage 91")
...
Debug.Trace(THIS_FILE + "-- remove the fragment Frag05 (last frament) from players inventory")
Game.GetPlayer().RemoveItem(Alias_Frag05.GetReference())

; run scene of Fjol call
Debug.Trace(THIS_FILE + "-- Run Scene: DSilHand_M80_SceneFjolCallToSkyforge")
...
;END AUTOCAST
;BEGIN CODE
Debug.Trace(THIS_FILE + "#STAGE 110 => COMPLETE QUEST")

; init misc where you will talk to fjol about the diary
Debug.Trace(THIS_FILE + "-- Set DSilHand_mi03preHonYsgramor to 20")
...
Debug.Trace(THIS_FILE + "-- start stage 70")
...
Debug.Trace(THIS_FILE + "#STAGE 100")
kmyQuest.evaluateAllPackages()
;END CODE
EndFunction
;END FRAGMENT
...
Debug.Trace(THIS_FILE + "#STAGE 80")

; Prepare Kodlak Journal
Debug.Trace(THIS_FILE + "-- Prepare Kodlak's journal")
...
; start stage 40: Follow the man to Jorrvaskr.
...
Debug.Trace(THIS_FILE + "#STAGE 101")
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
; Scene DSilHand_M80_SceneArmistice2 intermediate stage
Debug.Trace(THIS_FILE + "#STAGE 61")
Debug.Trace(THIS_FILE + "-- starting stage 61")
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
; Scene DSilHand_M80_SceneArmistice2 intermediate stage
Debug.Trace(THIS_FILE + "#STAGE 63")
;END CODE
EndFunction
;END FRAGMENT
...
{String with this file name, to be used on logs.}
